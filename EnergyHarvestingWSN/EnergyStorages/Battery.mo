...
model Battery "Battery cell model with a static internal impedance and self discharge"
  ...
  EnergyHarvestingWSN.EnergyStorages.Components.CurrentIntegrator currentIntegrator(symbol = "Q") annotation(
...
initial equation
  currentIntegrator.Q = SOCini * cellParam.C0;
//initial equation
//  Qabs = 0;
...
//Voc.v = max(-Ri.v, cellParam.E0 - cellParam.Epol * C / Q + cellParam.Av * exp(-cellParam.Bv * (C-Q)) );
//isCriticalCharging = soc.SoC < 0.05 or soc.SoC > 0.95;
  ...
  connect(Voc.p, currentIntegrator.n) annotation(
    Line(points = {{0, -10}, {0, 0}}, color = {0, 0, 255}));
  connect(currentIntegrator.p, Ri.n) annotation(
    Line(points = {{0, 20}, {0, 40}, {20, 40}}, color = {0, 0, 255}));
  connect(soc.SoC, currentIntegrator.SoC) annotation(
    Line(points = {{-28, 40}, {-8, 40}, {-8, 22}}, color = {0, 0, 127}));
  connect(currentIntegrator.Q, soc.Q) annotation(
    Line(points = {{-11, 10}, {-60, 10}, {-60, 40}, {-52, 40}}, color = {0, 0, 127}));
  connect(Shepherd.Q, currentIntegrator.Q) annotation(
