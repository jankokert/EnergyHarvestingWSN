...
block SafeOperation
  ...
  Modelica.Blocks.Tables.CombiTable1Ds interpolate(final table = saveOpTable) annotation(Placement(transformation(extent = {{-60, -10}, {-40, 10}}, rotation = 0)));
  Modelica.Blocks.Logical.Greater greater annotation(Placement(transformation(extent = {{-10, -10}, {10, 10}})));
  
equation
  connect(interpolate.u, Vdd) annotation(Line(points = {{-62, 0}, {-110, 0}}, color = {0, 0, 127}, smooth = Smooth.None));
